        ...
        if (acc_req_valid_i) begin
            unique casez (acc_req_i.data_op)
                ...
                idma_inst64_snitch_pkg::DMUSER : begin
                    ...
                    if ((0 < AxiUserWidth) && (AxiUserWidth <= 32)) begin
                        idma_fe_req_d.burst_req.user[AxiUserWidth-1:0] =
                            acc_req_i.data_arga[AxiUserWidth-1:0];
                    end else if (AxiUserWidth > 32) begin
                        idma_fe_req_d.burst_req.user[31:0] = acc_req_i.data_arga[31:0];
                        idma_fe_req_d.burst_req.user[AxiUserWidth-1:32] =
                            acc_req_i.data_argb[AxiUserWidth-33:0];
                    end
                    acc_req_ready_o = 1'b1;
                    is_dma_op       = 1'b1;
                    dma_op_name     = "DMUSER";
