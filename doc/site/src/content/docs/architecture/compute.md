...
Transpose does not change transfer size (input bytes == output bytes). The transport layer retires an output beat only once every logical byte lane of that beat has been consumed by the write port (`cmp_consumed_q`), so one output beat may span several bus beats (a misaligned destination, a 4 KiB split) without over-advancing the engine. The legalizer (`ComputeTransposeShape`) accepts two shapes: a tiled-walk strip of at most one beat (`length <= StrbWidth`, the shape the midend emits), or one whole padded tile in a single burst (`length == NE * StrbWidth` with `M <= NE` and `N <= NE`), where the source is stored at a `StrbWidth` row pitch. The multi-beat shape additionally requires a beat-aligned destination: the write barrel shifter rotates, so a misaligned bus beat would draw from two consecutive compute beats at once, which the single registered compute output cannot serve. The source may be misaligned in either shape, since the per-lane dataflow buffer absorbs that. Any other multi-beat shape would need the tiled (col-tile, row-tile, row) feed order that a contiguous burst cannot express.
