...
iDMA uses a SystemVerilog testbench driven by **job files** that describe transfer sequences. The testbench compares hardware behavior against a byte-accurate golden model. The full job-file testbenches run under Questa or VCS. Verilator provides both synthesis-top lint/elaboration and behavioral simulation of selected self-checking testbenches. Job files live in `jobs/backend_<variant>/`.
...
Each backend variant has its own set of job files under `jobs/backend_<variant>/`:
...
| `zero_transfer.txt` | Zero-length transfer; run at both settings of `RejectZeroTransfers` |
| `zero_transfer_busy.txt` | Zero-length transfer issued while other transfers are in flight |
| `zero_then_real.txt` | Zero-length transfer followed by a real one; catches a zero-length job corrupting its successor |
| `zero_unaligned_src.txt` | Zero-length transfer with a misaligned source address |
| `zero_unaligned_dst.txt` | Zero-length transfer with a misaligned destination address |
...
Selected self-checking testbenches also have behavioral Verilator targets. Use
`idma_sim_vlt_<testbench>` to run every declared parameterization of one testbench,
or `idma_sim_vlt_all` to run all of them. Their independent build directories live
under `target/sim/vlt/`.

## Running Simulations

`idma.mk` wires three simulators. Questa and VCS run the full job-file testbenches. Verilator is used separately for lint/elaboration and for behavioral simulation of selected self-checking testbenches. Each flow takes its tool binaries as make variables, whose defaults are the bare `vsim` / `vcs` / `verilator` names; the Questa examples below use the `questa-2023.4` SEPP wrapper.

### Prerequisites

Generate the RTL (and, for Questa/VCS, the compile scripts) first:

```bash
make idma_hw_all    # Generate RTL into target/rtl/
make idma_sim_all   # Generate compile.tcl + start.tcl (Questa) and compile.sh (VCS)
```

### Compile and Run

<Tabs syncKey="sim">
  <TabItem label="Questa">

  Compile with the generated `compile.tcl`, then run a job file with the `+job_file` plusarg:
  ...
  Analyze the sources (this generates and runs `target/sim/vcs/compile.sh`), elaborate a specific testbench into a `simv` binary, then run it with the same `+job_file` plusarg:

  ```bash
  make idma_vcs_compile VCS=vcs VLOGAN=vlogan
  make target/sim/vcs/bin/rw_axi.vcs IDMA_VCS_TB=tb_idma_backend_rw_axi
  cd target/sim/vcs
  ./bin/rw_axi.vcs +job_file=../../../jobs/backend_rw_axi/simple.txt
  ```

  `IDMA_VCS_TB` selects the testbench top; the `bin/<name>.vcs` output name is yours to choose.

  </TabItem>
  <TabItem label="Verilator">

  The lint flow checks generated synthesis tops and writes its intermediate files to
  `target/lint/verilator/`:

  ```bash
  make idma_lint_all VERILATOR=verilator
  make idma_lint_params IDMA_TOP=idma_backend_synth_rw_axi VERILATOR=verilator
  ```

  The behavioral flow builds executable models in per-configuration directories
  under `target/sim/vlt/`. Friendly targets run all parameterizations declared for
  a testbench:

  ```bash
  make idma_sim_vlt_tb_idma_transpose_nd VERILATOR=verilator
  make idma_sim_vlt_all VERILATOR=verilator
  ```

  A single parameterization can be selected with its generated run target, for
  example `idma_vlt_run_tb_idma_transpose_nd__dw64`. Set `TRACE=1` for FST tracing,
  or `TRACE=2` to additionally trace structs.
