...
`idma.mk` wires three simulators. Questa and VCS run the full job-file testbenches; Verilator is wired for elaboration/lint only (it Verilates a chosen top with `--cc`, not a full behavioral run). Each flow takes its tool binaries as make variables, whose defaults are the bare `vsim` / `vcs` / `verilator` names; the Questa examples below use the `questa-2023.4` SEPP wrapper.
...
<Tabs syncKey="sim">
  ...
  Verilator is wired for elaboration / lint only (`--cc`), not a full job-file run. It pickles the design with `-D VERILATOR` and Verilates a chosen top, writing the result to `target/sim/verilator/<top>_elab.log`:

  ```bash
  make target/sim/verilator/idma_backend_synth_rw_axi_elab.log \
    IDMA_VLT_TOP=idma_backend_synth_rw_axi VERILATOR=verilator
  ```

  Use this as a fast structural / lint check; run Questa or VCS for behavioral job-file simulation.
