...
module idma_inst64_base #(
    ...
    // Every instantiated rule must name a real window: an all-zero rule is NOT a miss,
    // cc_addr_decode reads end_addr == 0 as "end of address space" and would route every
    // address to TCDMDMA (OBI). The alias rule therefore only exists when it is enabled.
    // Anything outside the mapped windows falls back to default_idx_i = ToSoC, i.e. AXI.
