...
// Drives the APB config slave against a controllable backend stub and gates the
// next_id launch deadlock with a per-read watchdog.
...
module tb_idma_reg_frontend import idma_pkg::*; #(
  ...
  // config-clock cycles is a deadlock (the fix completes in a handful of cycles).
  ...
  // `accept` and `do_retire` cannot both change `outstanding` inconsistently
  // because all updates happen in this one process.
  ...
  // Strict APB4 master. Signals are driven just after a negedge (stable before the
  // sampling posedge); pready/prdata are sampled at the posedge of the ACCESS phase.
  // On the posedge that pready is high the transaction retires and the bus returns to
  // IDLE at that same edge (psel/penable deasserted), then one guaranteed idle cycle
  // is inserted before the next transaction. This is what keeps the reg_top APB FSM
  // (which starts a transfer on psel, and clears is_active on rd/wr_ack) from being
  // re-triggered into a spurious second access — one APB transaction == one CPUIF req.
  task automatic apb_xact(input  bit          write,
    ...
    // ACCESS phase: raise penable, then wait for pready at the posedge. The bus is
    // held stable (psel & penable & addr/data) across every stalled cycle — exactly
    // the sustained handshake a deadlocked launch would hang on.
    @(negedge clk);
  ...
  task automatic apb_write(input logic [31:0] addr, input logic [31:0] data);
    logic [31:0] dummy;
    apb_xact(1'b1, addr, data, dummy);
  endtask

  // APB4 read that RESPECTS pready: not complete until pready is high. If
  // `is_next_id`, the watchdog net tracks the outstanding read so a frozen launch
  // trips $fatal.
  task automatic apb_read(input logic [31:0] addr,
                          output logic [31:0] data,
                          input bit is_next_id = 1'b0);
    apb_xact(1'b0, addr, 32'h0, data, is_next_id);
  endtask
  ...
  // launch: read next_id (this is the transfer trigger); returns the id
  task automatic launch(output logic [31:0] id, input int unsigned s = 0);
    apb_read(reg_next_id(s), id, .is_next_id(1'b1));
  endtask

  task automatic read_done(output logic [31:0] id, input int unsigned s = 0);
    apb_read(reg_done_id(s), id);
  endtask
  ...
  initial begin : test
    ...
    // ------------------------------------------------------------------
    // Test 1 — Basic launch (backend ready)
    // ------------------------------------------------------------------
    $display("\n--- Test 1: basic launch ---");
    backend_auto_retire = 1'b1;
    req_ready           = 1'b1;
    ...
    launch(id0);
    // the launch returns exactly the id that was pending, and issues it once
    ...
    // ------------------------------------------------------------------
    // Test 2 — DEADLOCK / backpressure regression (core gate)
    // ------------------------------------------------------------------
    $display("\n--- Test 2: backpressure / deadlock regression ---");
    ...
    exp_id = next_id;                 // id that this (still ungranted) launch must return
    fork
      begin : do_launch
        // this read MUST stall (pready low) while req_ready is low, then
        // complete the cycle the grant lands — the watchdog guards a freeze.
        launch(id1);
      end
      begin : release_bp
        int unsigned stall_obs;
        stall_obs = 0;
        // observe the launch genuinely stalling for K cycles without freezing
        repeat (40) begin
          @(posedge clk);
          if (nxt_read_active && !apb_rsp[0].pready) stall_obs++;
        end
        check_eq(stall_obs >= 32'd30, 1'b1, "Test2 read stalled (pready low) under BP");
        // the id must NOT have advanced while the launch was held ungranted
        check_eq(next_id, exp_id, "Test2 id held (no issue) during stall");
        // release backpressure — the held launch must now complete
        req_ready = 1'b1;
      end
    join
    check_eq(id1, exp_id, "Test2 launch id after stall == pre-stall next_id");
    ...
    if (NumStreams > 1) begin
      int unsigned held1_cnt;          // cycles stream_idx==1 while the latch is engaged
      logic        bad_idx;            // stream_idx ever pointed at a wrong stream
      $display("\n--- Test 3: multi-stream stream_idx held under BP ---");
      ...
      fork
        begin : ms_launch
          launch(id2, 1);              // launch on stream 1
        end
        begin : ms_check
          // While the launch is genuinely stalled (the arbiter is presenting the
          // request, req_valid_o high, but pready is still low), stream_idx_o MUST
          // read 1 for the whole stall — exactly the latent bug the pending latch
          // corrected: the maskable req dropped stream_idx to 0 mid-stall. This is
          // gated on PORTS only (req_valid_o + pready), so it elaborates against the
          // buggy template too and is not coupled to any internal signal name.
          repeat (30) begin
            @(posedge clk);
            if (nxt_read_active && req_valid && !apb_rsp[0].pready) begin
              if (stream_idx == 1) held1_cnt++;
              else                 bad_idx = 1'b1;   // wrong / dropped stream index
            end
          end
          req_ready = 1'b1;            // release
        end
      join
      // held for essentially the whole 30-cycle window, never pointing elsewhere
      check_eq(held1_cnt >= 32'd20, 1'b1, "Test3 stream_idx held == 1 across stall");
      ...
      prev_id = id2;
      check_eq(captured_q.size(), 32'd1, "Test3 stream1 accepted once");
      backend_auto_retire = 1'b1;
      poll_done(id2, 1);
    ...
    begin
      ...
      // exactly four accepted launches, each id one more than the last (distinct,
      // monotonic, none dropped, none doubled)
