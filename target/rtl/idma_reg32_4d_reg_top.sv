...
module idma_reg32_4d_reg_top (
    ...
    // Field: idma_reg.compute_cfg.transpose_compact
    always_comb begin
        automatic logic [0:0] next_c;
        automatic logic load_next_c;
        next_c = field_storage.compute_cfg.transpose_compact.value;
        load_next_c = '0;
        if(decoded_reg_strb.compute_cfg && decoded_req_is_wr) begin // SW write
            next_c = (field_storage.compute_cfg.transpose_compact.value & ~decoded_wr_biten[31:31]) | (decoded_wr_data[31:31] & decoded_wr_biten[31:31]);
            load_next_c = '1;
        end
        field_combo.compute_cfg.transpose_compact.next = next_c;
        field_combo.compute_cfg.transpose_compact.load_next = load_next_c;
    end
    always_ff @(posedge clk or negedge arst_n) begin
        if(~arst_n) begin
            field_storage.compute_cfg.transpose_compact.value <= 1'h0;
        end else begin
            if(field_combo.compute_cfg.transpose_compact.load_next) begin
                field_storage.compute_cfg.transpose_compact.value <= field_combo.compute_cfg.transpose_compact.next;
            end
        end
    end
    assign hwif_out.compute_cfg.transpose_compact.value = field_storage.compute_cfg.transpose_compact.value;
    ...
    always_comb begin
        ...
        if(rd_mux_addr == 9'h104) begin
            ...
            readback_data_var[31] = field_storage.compute_cfg.transpose_compact.value;
        end
